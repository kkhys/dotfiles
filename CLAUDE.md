...
- Codex — `.config/codex/rules/managed.rules` (execpolicy `prefix_rule`; allow / prompt / forbidden ≙ allow / ask / deny, except for the irreversible part of `ask` noted below). Verify with `codex execpolicy check --rules <file> -- <command>`; a rule's `match` / `not_match` examples are checked whenever the file is loaded, so a wrong prefix fails the command rather than passing silently. The `codexRulesSync` activation in `home-manager/dotfiles.nix` copies it to `~/.codex/rules/managed.rules` as a regular file on every `darwin-rebuild switch` — Codex skips symlinked `*.rules`, so it must not be a `home.file` link — and Codex loads rules only at startup, so restart it after a switch
...
- Codex answers a `prompt` decision with the reviewer subagent, not a person (`approvals_reviewer = "auto_review"` in `.config/codex/config.toml`), so Claude `ask` entries that a wrong answer cannot undo — `git push --force`, `git reset --hard`, `git clean`, `gh pr merge`, `sudo`, `npm`/`pnpm`/`cargo publish`, `kubectl delete`, `aws s3 mv`, `aws ec2 run/start/stop-instances`, `aws lambda update-function-code`, `aws cloudformation create/update-stack` — are `forbidden` there while staying `ask` in the Devin and Cursor mirrors, which still prompt a person. Changing `approvals_reviewer` back to `user` is what would justify demoting them to `prompt` again.
- Codex splits a shell script into its component commands only when it is a linear chain of plain words joined by `&&`, `||`, `;` or `|`; a redirect, substitution, variable or glob anywhere in it makes the whole script one `zsh -lc <script>` argv that no `prefix_rule` matches. Such invocations always reach the reviewer, whatever the tiers say. Note that `codex execpolicy check` does not perform that split at all, so test rules with the bare argv (`-- gh pr view 1`), never through a shell wrapper.
